...
Public Sub CalibrateTelescope(InitTimerad As Double, Star1 As RaDec, Tel1 As AzAlt, ObservTime1Rad As Double, Star2 As RaDec, Tel2 As AzAlt, ObservTime2Rad As Double, TransformationMatrix() As Double)
    ...
    'Equation (5.4-5)
    lmn_Tel_1.x = Cos(Tel1.Alt) * Cos(Tel1.Az)
    lmn_Tel_1.Y = Cos(Tel1.Alt) * Sin(Tel1.Az)
    lmn_Tel_1.z = Sin(Tel1.Alt)

    'Equation (5.4-6)
    LMN_Equ_1.x = Cos(Star1.Dec) * Cos(Star1.Ra - SidConst * (ObservTime1Rad - InitTimerad))
    LMN_Equ_1.Y = Cos(Star1.Dec) * Sin(Star1.Ra - SidConst * (ObservTime1Rad - InitTimerad))
    LMN_Equ_1.z = Sin(Star1.Dec)

    'Equation (5.4-7)
    lmn_Tel_2.x = Cos(Tel2.Alt) * Cos(Tel2.Az)
    lmn_Tel_2.Y = Cos(Tel2.Alt) * Sin(Tel2.Az)
    lmn_Tel_2.z = Sin(Tel2.Alt)

    'Equation (5.4-8)
    LMN_Equ_2.x = Cos(Star2.Dec) * Cos(Star2.Ra - SidConst * (ObservTime2Rad - InitTimerad))
    LMN_Equ_2.Y = Cos(Star2.Dec) * Sin(Star2.Ra - SidConst * (ObservTime2Rad - InitTimerad))
    LMN_Equ_2.z = Sin(Star2.Dec)
...
Public Sub CalculateTelescopeCoordinates(InitTimerad As Double, CurrStarRad As RaDec, AimTimeRad As Double, TransformationMatrix() As Double, AzAlt_CurrStar As AzAlt)
    ...
    LMN_Equ_Result.x = Cos(CurrStarRad.Dec) * Cos(CurrStarRad.Ra - SidConst * (AimTimeRad - InitTimerad))
    LMN_Equ_Result.Y = Cos(CurrStarRad.Dec) * Sin(CurrStarRad.Ra - SidConst * (AimTimeRad - InitTimerad))
    LMN_Equ_Result.z = Sin(CurrStarRad.Dec)
